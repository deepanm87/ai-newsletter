      ...
      try {
        ...
        if (response.ok) {
          ...
          if (data.feedsToRefresh > 0) {
            toast.info(`Refreshing ${data.feedsToRefresh} feed${data.feedsToRefresh > 1 ? "s" : ""}...`)
          }

          if (data.articlcesFound > 0) {
            toast.info(`Analyzing ${data.articlesFound} articles${data.articlesFound > 1 ? "s" : ""} from your feeds...`)
